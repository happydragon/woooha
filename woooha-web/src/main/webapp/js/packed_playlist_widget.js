(function (b) {
    var a = {}, c = function (i, f, e, d, h, g, k) {
        this.id = i;
        this.name = f;
        this.url = e;
        this.rawUrl = d;
    ...
    c.prototype.getLyrics = function (e) {
        var d = this;
        if (!this.lyrics.length) {
            $.getJSON(c.options.lyrics_src, {sid:this.id}, function (f) {
                if (f.status === 0) {
                    d.lyrics = f.lyrics
                }
                e(f)
            ...
            e({status:0, lyrics:d.lyrics})
        }
    };
    c.prototype.addComment = function (g, f, d) {
        var e = this;
        $.post_withck(c.options.add_comment_src, {sid:e.id, comment:g}, function (h) {
            if (h.status == 0) {
                f(h)
            } else {
                d(h)
    ...
    c.prototype.getComments = function (d) {
        $.getJSON(c.options.comments_src, {sid:this.id}, d)
    };
    c.options = {};
    c.config = function (d) {
        $.extend(c.options, d)
    };
    c.getSong = function (d) {
        return a[d]
    };
    c.create = function (j, f, e, d, h, g, k) {
        var i = a[j];
        if (!i) {
            i = a[j] = new c(j, f, e, d, h, g, k)
        }
        return i
    };
    c.deleteComment = function (g, d, f) {
        var e = c.options.del_comment_src;
        $.post(e, {comment_id:g, ck:d}, f, "json")
    };
    b.Song = c
})(window);
$.fn.shareToSina = function () {
    var h = this.attr("sina-appkey") || window.appkey, j = this.attr("sina-screen") || window.screen, i = this.attr("sina-document") || window.document, f = this.attr("sina-encode") || window.encodeURIComponent, a = this.attr("sina-source") || "", b = this.attr("sina-source_url") || "", c = this.attr("sina-picURL"), g = this.attr("sina-content"), d = this.attr("sina-url"), e = this.attr("sina-encoding") || "utf-8";
    (function (C, v, q, k, n, m, B, x, w) {
        var o = "http://v.t.sina.com.cn/share/share.php?appkey=" + h, A = x || v.location, m = ["&url=", q(A), "&title=", q(B || v.title), "&source=", q(k), "&sourceUrl=", q(n), "&content=", w || "gb2312", "&pic=", q(m || "")].join("");

        function y() {
            if (!window.open([o, m].join(""), "mb", ["toolbar=0,status=0,resizable=1,width=440,height=430,left=", (C.width - 440) / 2, ",top=", (C.height - 430) / 2].join(""))) {
                A.href = [o, m].join("")
            }
        }
...
(function (a) {
    var c = {}, b = function (f, d, e) {
        this.id = f;
        this.songs = d || [];
        this.currentSongIndex = e || 0
    };
    b.fn = b.prototype;
    ...
    b.findOrCreate = function (d) {
        return b.find(d) || b.create(d)
    };
    b.fn.addSong = function (d) {
        this.songs.push(d)
    };
    b.fn.getNextSong = function () {
        var d = ++this.currentSongIndex % this.songs.length;
        return this.songs[d]
    };
    b.fn.getCurrentSong = function () {
        return this.songs[this.currentSongIndex]
    };
    b.fn.hasNextSong = function () {
        return this.currentSongIndex < (this.songs.length - 1)
    };
    b.fn.findSong = function (e) {
        for (var d in this.songs) {
            if (this.songs[d].id === e) {
                return d
            }
        }
        return -1
    };
    b.fn.setCurrentSong = function (d) {
        this.currentSongIndex = d
    };
    a.PlaylistWidget = b
})(window);
(function (i, f) {
...
function playcount(b, a) {
    $.get("/j/artist/incplaycount", {song_id:b, worked:a ? 1 : 0})
}
window.playNext = function (player_id) {
    var widget = PlaylistWidget.findOrCreate(player_id), hasNext = widget.hasNextSong(), song = widget.getNextSong();
    playSong(player_id, song, !hasNext);
};
window.playSong = function (player_id, song, noThisSong) {
    var player = $("#mplayer_" + player_id)[0], c = $(".table_" + player_id);
    $("tr", c).removeClass("selsong");
    if (!noThisSong) {
        $(".tr" + song.id, c).addClass("selsong")
    }
    if (player !== undefined && player.playsong !== undefined) {
        player.playsong(song.url, song.id, noThisSong ? 0 : 1, song.name, song.isDemo, song.singer, song.cover)
    }
};